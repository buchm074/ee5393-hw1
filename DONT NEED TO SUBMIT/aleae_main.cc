...
int main(int argc, char **argv) 
{
   ...
   if (argc < 6 || argc > 7) {
      cerr << "usage: " << argv[0]
     << " <file1: state> <file2: reactions> <trials> <time> <verbosity> [maxSteps]"
     << endl;
   } 
   ...
   param.event_lt = 0;
   if (argc == 7) {
      param.event_lt = atoi(argv[6]);
   }

   // for each trial
   vector<unsigned> A(S.size(), 0);
   vector<unsigned long long> A2(S.size(), 0); 
   for (unsigned n = 0; n < trials; n++) {
      ...
      for (unsigned i = 0; i < A.size(); i++) {
         A[i]  += I[i];
         A2[i] += (unsigned long long)I[i] * (unsigned long long)I[i];
      }
   ...
   for (unsigned i = 0; i < A.size(); i++) {
      ...
      double var  = A2[i] / (double)trials - mean * mean;   // population variance
      cout << N[i] << " mean=" << mean << " var=" << var << endl;
   }
   for (unsigned i = 0; i < T.size(); i++) {
      cout <<  N[T[i].i];
      switch(T[i].c) {
